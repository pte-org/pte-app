...
void main() {
  ...
  blocTest<AuthBloc, AuthState>(
    ...
    build: buildBloc,
    act: (bloc) => bloc.add(const LoginRequested(email: 'a@b.com', password: 'secret')),
    expect: () => [
      isA<AuthAuthenticating>(),
      isA<AuthAuthenticated>().having((s) => s.claims.roles, 'roles', ['STUDENT']),
    ],
    verify: (_) {
  ...
  blocTest<AuthBloc, AuthState>(
    ...
    build: buildBloc,
    act: (bloc) => bloc.add(const LoginRequested(email: 'a@b.com', password: 'wrong')),
    expect: () => [
      isA<AuthAuthenticating>(),
  ...
  blocTest<AuthBloc, AuthState>(
    ...
    build: buildBloc,
    seed: () => const AuthAuthenticated(JwtClaims(roles: ['STUDENT'], tenantId: 't1')),
