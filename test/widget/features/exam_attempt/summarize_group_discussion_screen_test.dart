...
/// `SummarizeGroupDiscussionScreen` constructs its own `AutoRecordCubit`
...
/// Fixture matches the app's own dev fixture: `prepSeconds: 200` is the sum
/// of the 3 mocked sub-stages (5s pre-listen + 185s audio + 10s pre-record),
/// `responseSeconds: 120` is the recording window.
TaskView _summarizeGroupDiscussionTask({String pinnedItemPublicId = 'item-1'}) {
  return TaskView(
...
void main() {
  ...
  Widget buildSubject({TaskView? task}) {
    return MaterialApp(
      home: BlocProvider<ExamAttemptBloc>.value(
        value: bloc,
        child: SummarizeGroupDiscussionScreen(
          task: task ?? _summarizeGroupDiscussionTask(),
  ...
  testWidgets('renders the fixed instruction text (no interpolated variables)', (
    ...
    const snapshot = TimerSnapshot(
      ...
      remaining: Duration(seconds: 200),
      currentOrderIndex: 1,
    );
    stubBlocState(
      AttemptInProgress('attempt-1', _summarizeGroupDiscussionTask(), snapshot),
    );
  ...
  group(
    ...
    () {
      testWidgets(
        'elapsed 0s (start of pre-listen prep): Listening shows "Beginning in 5", Record is blank',
        (tester) async {
          const snapshot = TimerSnapshot(
            phase: TimerPhase.prep,
            remaining: Duration(seconds: 200),
          ...
          expect(find.text('Beginning in 5 seconds'), findsOneWidget);
          expect(
            find.text('Volume'),
            findsOneWidget,
      ...
      testWidgets(
        'elapsed 5s (start of audio playback): Listening shows "Playing 185 seconds left", Record blank',
        (tester) async {
          // prepSeconds 200 - elapsed 5 = remaining 195.
          const snapshot = TimerSnapshot(
            phase: TimerPhase.prep,
            remaining: Duration(seconds: 195),
            currentOrderIndex: 1,
          );
          stubBlocState(
            AttemptInProgress(
          ...
          expect(find.text('Playing 185 seconds left'), findsOneWidget);
      ...
      testWidgets(
        'elapsed 190s (boundary into pre-record prep, listening finished): Listening frozen at "Playing 0", '
        'Record shows "Beginning in 10"',
        (tester) async {
          // prepSeconds 200 - elapsed 190 = remaining 10.
          const snapshot = TimerSnapshot(
            phase: TimerPhase.prep,
      ...
      testWidgets(
        'elapsed 199s (mid pre-record prep): Record shows "Beginning in 1"',
        (tester) async {
          // prepSeconds 200 - elapsed 199 = remaining 1.
          const snapshot = TimerSnapshot(
            phase: TimerPhase.prep,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      stubBlocState(
        AttemptInProgress('attempt-1', _summarizeGroupDiscussionTask(), prep),
      );
      ...
      const response = TimerSnapshot(
        ...
        remaining: Duration(seconds: 120),
        currentOrderIndex: 1,
      );
      stateController.add(
        AttemptInProgress(
      ...
      expect(find.text('Playing 0 seconds left'), findsOneWidget);
      expect(find.text('Recording 120 seconds left'), findsOneWidget);
    },
  );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('Recording 120 seconds left'), findsNothing);
      expect(find.text('Playing 0 seconds left'), findsOneWidget);
    },
  );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      stateController.add(
        AttemptInProgress(
          'attempt-1',
          _summarizeGroupDiscussionTask(pinnedItemPublicId: 'item-2'),
          otherTaskResponse,
        ),
      );
      await tester.pump();
