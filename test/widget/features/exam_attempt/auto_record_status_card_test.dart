...
void main() {
  Widget buildSubject({required AutoRecordState recordingState, required TimerSnapshot? snapshot}) {
    return MaterialApp(
      home: Scaffold(
        body: AutoRecordStatusCard(task: _task(), recordingState: recordingState, snapshot: snapshot),
      ),
    );
  }

  testWidgets('prep phase shows a live "Beginning in…" countdown with progress forced to 0.0', (tester) async {
    const snapshot = TimerSnapshot(phase: TimerPhase.prep, remaining: Duration(seconds: 10), currentOrderIndex: 1);

    await tester.pumpWidget(buildSubject(recordingState: const AutoRecordState(), snapshot: snapshot));
  ...
  testWidgets('response phase shows the live "Recording… seconds left" label with the correct elapsed fraction', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      buildSubject(
        recordingState: const AutoRecordState(recordingPhase: RecordingPhase.recording),
        snapshot: snapshot,
      ),
    );
  ...
  testWidgets('recorded phase always wins regardless of the live snapshot phase, progress 1.0', (tester) async {
    const snapshot = TimerSnapshot(phase: TimerPhase.response, remaining: Duration(seconds: 5), currentOrderIndex: 1);

    await tester.pumpWidget(
      buildSubject(
        recordingState: const AutoRecordState(recordingPhase: RecordingPhase.recorded),
        snapshot: snapshot,
      ),
    );
