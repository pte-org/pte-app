...
/// same fully-automatic advance, same 2-card layout) — only the sub-stage
/// constants and instruction text differ. The `prep` phase covers pre-listen
/// ([_preListenSeconds]) → mocked audio → pre-record ([_preRecordSeconds]);
/// both [AudioListeningStatusCard] and [RecordedAnswerStatusCard] stay
/// visible throughout, each running its own independent countdown.
...
class _ListeningCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final audioSeconds =
        (task.prepSeconds - _preListenSeconds - _preRecordSeconds).clamp(
          0,
          task.prepSeconds,
        );
...
class _RecordCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final elapsed = _elapsedPrepSeconds(task, snapshot);
    final preRecordStart = (task.prepSeconds - _preRecordSeconds).clamp(
      0,
      task.prepSeconds,
    );
